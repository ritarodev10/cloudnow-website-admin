...
import { MoreHorizontalIcon, EditIcon, CopyIcon, TrashIcon, EyeIcon, UsersIcon, LinkIcon } from "lucide-react";
              ...
              <TableRow key={group.id} className="hover:bg-muted/50">
                ...
                <TableCell>
                  <Badge variant="secondary">
                    {group.testimonialIds.length} testimonial{group.testimonialIds.length !== 1 ? "s" : ""}
                  </Badge>
                </TableCell>
                    ...
                    <span className="text-sm">
                      {group.usagePaths.length > 0 ? (
                        <Badge variant="outline" className="text-xs">
                          {group.usagePaths.length} page{group.usagePaths.length !== 1 ? "s" : ""}
                        </Badge>
                      ) : (
                  ...
                  {group.usagePaths.length > 0 && (
                    <div className="mt-1 text-xs text-muted-foreground">
                      ...
                      {group.usagePaths.length > 2 && (
                        <div className="text-muted-foreground">+{group.usagePaths.length - 2} more</div>
                      )}
                    </div>
                  )}
                </TableCell>
                ...
                <TableCell>
                  <div className="text-sm text-muted-foreground">{formatDate(group.updatedAt)}</div>
                </TableCell>
